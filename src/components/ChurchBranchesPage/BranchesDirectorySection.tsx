...
const NATIONAL_BRANCHES: ChurchBranch[] = [
  {
    id: 'in-1',
...
const INTERNATIONAL_BRANCHES: ChurchBranch[] = [
  {
    id: 'intl-1',
...
export const BranchesDirectorySection: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'national' | 'international'>('national')
  const [searchQuery, setSearchQuery] = useState('')

  const currentDataset = activeTab === 'national' ? NATIONAL_BRANCHES : INTERNATIONAL_BRANCHES
    ...
    <section className="relative py-4 sm:py-8 md:py-10 bg-[#f8fafc]" data-node-id="286:2998">
      ...
      <div className="max-w-[1140px] mx-auto px-4 sm:px-6 lg:px-0">
        ...
        {filteredBranches.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-[20px] sm:gap-y-[25px] gap-x-[16px] sm:gap-x-[19px]">
            {filteredBranches.map((branch) => (
              <div
                key={branch.id}
                className="w-full max-w-[367px] mx-auto min-h-[175px] sm:min-h-[185px] bg-[#e2e8f0] rounded-[20px] p-5 sm:p-6 flex flex-col items-center justify-center text-center shadow-sm hover:shadow transition-shadow"
              >
                {/* Circular Navy Pin Icon (Figma 286:3133: 51px x 51px) */}
                <div className="w-[44px] h-[44px] sm:w-[51px] sm:h-[51px] rounded-full bg-[#003370] flex items-center justify-center mb-2.5 sm:mb-3 shadow-inner">
                ...
                <h3 className="font-poppins font-semibold text-[#003370] text-base sm:text-[20px] leading-tight mb-1 uppercase tracking-tight">
                  {branch.name}
                </h3>
